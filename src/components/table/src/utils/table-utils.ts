...
export interface RsTableColumnEditorOptions {
   ...
   */
  options?:
    | import('../../../select/src/select-utils').RsSelectOptions
    | ((row: RsTableRowData, index: number) => import('../../../select/src/select-utils').RsSelectOptions)
  /** select：可搜索 */
  searchable?: boolean
  /**
...
export type RsTableColumnEditorOptionsResolved = Omit<RsTableColumnEditorOptions, 'options'> & {
  options?: import('../../../select/src/select-utils').RsSelectOptions
}
...
export interface RsTableColumn<T extends RsTableRowData = Record<string, unknown>> {
   ...
   */
  summary?: import('./table-summary-utils').RsTableColumnSummary<T>
}
