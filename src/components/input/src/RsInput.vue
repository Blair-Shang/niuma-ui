...
import { type RsInputRule, type RsInputValidateTrigger } from './input-rules'
import {
...
export type { RsInputType }

defineOptions({ name: 'RsInput', inheritAttrs: false })
...
const hasAddonAfterContent = computed(() =>
  hasRsInputAddonAfterContent(Boolean(slots.addonAfter), props.addonAfter),
)
const hasAddonAfter = computed(() => hasAddonAfterContent.value || Boolean(props.addonAfterIcon))
...
const resolvedAutocomplete = computed(() =>
  resolveRsInputAutocomplete(props.autocomplete, props.type),
)
...
const resolvedShowPassword = computed(() => props.type === 'password' && props.visibilityToggle)
const resolvedInputType = computed(() =>
  resolveRsInputControlType(props.type, passwordVisible.value),
)
...
<template>
  ...
  >
    ...
    <div class="rs-input-field">
      ...
      >
        <span v-if="hasAddonBefore" class="rs-input-addon rs-input-addon--before">
        ...
        >
          <span v-if="hasPrefix" class="rs-input-group__affix rs-input-group__affix--prefix">
          ...
          <input
            ...
            @keydown="onKeydown"
          />
          <span v-if="hasSuffix" class="rs-input-group__affix rs-input-group__affix--suffix">
            <span v-if="showCountDisplay" class="rs-input-group__count" aria-hidden="true">
              {{ countText }}
            </span>
            <button
    ...
    <p v-if="displayMessage" :id="errorId" class="rs-input-field__error" role="alert">
