import type { RsLocaleMessages } from './types'

export const zhCN: RsLocaleMessages = {
  'select.placeholder': '请选择',
  'select.searchPlaceholder': '搜索…',
  'select.empty': '无匹配结果',
  'select.loading': '加载中…',
  'select.clear': '清空',
  'select.createOption': '使用 “{query}”',
  'select.maxTagPlaceholder': '+{count}',
  'select.listbox': '选项列表',
  'mentions.placeholder': '输入 @ 提及',
  'mentions.suggestions': '提及建议',
  'autocomplete.placeholder': '输入以搜索',
  'autocomplete.suggestions': '自动完成建议',
  'dropdown.placeholder': '请选择',
  'dropdown.menu': '下拉菜单',
  'contextMenu.menu': '右键菜单',
  'contextMenu.submenu': '子菜单',
  'breadcrumb.label': '面包屑',
  'breadcrumb.separator': '分隔符',
  'breadcrumb.more': '显示更多层级',
  'anchor.label': '本页目录',
  'steps.label': '步骤',
  'steps.wait': '未开始',
  'steps.process': '进行中',
  'steps.finish': '已完成',
  'steps.error': '出错',
  'menu.label': '导航菜单',
  'menu.expandSubmenu': '展开子菜单',
  'menu.collapseSubmenu': '收起子菜单',
  'sidebar.label': '侧栏',
  'sidebar.nav': '侧栏导航',
  'sidebar.expand': '展开侧栏',
  'sidebar.collapse': '收起侧栏',
  'toolbar.label': '工具栏',
  'input.required': '此项为必填',
  'label.required': '必填',
  'label.optional': '（选填）',
  'label.colon': '：',
  'form.field': '此项',
  'form.validate.required': '请填写{label}',
  'form.list.add': '添加',
  'form.list.remove': '删除',
  'fieldset.help': '分组说明',
  'tooltip.help': '说明',
  'popover.label': '气泡卡片',
  'popover.close': '关闭',
  'input.formatInvalid': '格式不正确',
  'input.rule.email': '请输入有效的邮箱地址',
  'input.rule.number': '请输入有效数字',
  'input.rule.integer': '请输入整数',
  'input.rule.phone': '请输入有效的 11 位手机号',
  'input.rule.url': '请输入有效的 URL（需以 http:// 或 https:// 开头）',
  'input.rule.minComplexity': '密码至少 8 位，且包含字母与数字',
  'input.clear': '清除',
  'input.showPassword': '显示密码',
  'input.hidePassword': '隐藏密码',
  'inputNumber.increase': '增加',
  'inputNumber.decrease': '减少',
  'common.confirm': '确认',
  'common.cancel': '取消',
  'common.remove': '移除',
  'dialog.close': '关闭',
  'dialog.fullscreen': '全屏',
  'dialog.restore': '还原',
  'dialog.label': '对话框',
  'tag.close': '关闭标签',
  'scrollbar.viewport': '滚动区域',
  'virtualList.empty': '暂无数据',
  'virtualList.label': '虚拟列表',
  'monaco.label': '代码编辑器',
  'alert.close': '关闭提示',
  'empty.description': '暂无数据',
  'descriptions.empty': '—',
  'empty.search': '没有匹配的结果',
  'empty.label': '空状态',
  'dynamicTags.label': '标签编辑',
  'dynamicTags.placeholder': '输入后回车添加',
  'dynamicTags.add': '添加标签',
  'dynamicTags.clear': '清空标签',
  'dynamicTags.duplicate': '标签已存在',
  'dynamicTags.maxReached': '已达数量上限',
  'dynamicTags.invalid': '标签无效',
  'confirm.title': '确认操作？',
  'confirm.description': '此操作可能无法撤销，请确认后继续。',
  'loading.label': '加载中',
  'statCard.loading': '加载中',
  'statCard.trend.up': '上升',
  'statCard.trend.down': '下降',
  'statCard.trend.flat': '持平',
  'loadingBar.label': '正在加载',
  'loadingBar.error': '加载失败',
  'loadingBar.indeterminate': '正在加载，进度未知',
  'toaster.region': '通知',
  'toaster.close': '关闭通知',
  'toaster.loading': '正在处理',
  'toaster.done': '已完成',
  'toaster.failed': '操作失败',
  'tabs.label': '标签页',
  'tabs.add': '新建标签页',
  'tabs.close': '关闭 {label}',
  'tabs.closeTab': '关闭',
  'tabs.closeOthers': '关闭其他',
  'tabs.closeLeft': '关闭左侧',
  'tabs.closeRight': '关闭右侧',
  'tabs.closeAll': '关闭全部',
  'tabs.more': '更多',
  'tabs.moreCount': '更多 ({count})',
  'tabs.rename': '重命名标签页',
  'tabs.renamePlaceholder': '输入标签名称',
  'tabs.scrollPrev': '向前滚动标签',
  'tabs.scrollNext': '向后滚动标签',
  'tabs.dragHandle': '拖动排序',
  'pagination.label': '分页',
  'pagination.prev': '上一页',
  'pagination.next': '下一页',
  'pagination.first': '首页',
  'pagination.last': '末页',
  'pagination.page': '页',
  'pagination.pageN': '第 {page} 页',
  'pagination.current': '第 {page} 页，当前页',
  'pagination.simple': '{page} / {pageCount}',
  'pagination.ellipsis': '更多页',
  'pagination.summary': '{total, plural, other {共 # 条}}',
  'pagination.jumpTo': '跳至',
  'pagination.jumpPageSuffix': '页',
  'pagination.jumpConfirm': '确定',
  'table.loading': '加载中…',
  'table.empty': '暂无数据',
  'table.loadingMore': '加载更多…',
  'table.a11yLabel': '数据表格',
  'table.selectAll': '全选',
  'table.selectRow': '选择行',
  'table.index': '#',
  'table.expandRow': '展开行',
  'table.collapseRow': '收起行',
  'table.dragColumn': '拖拽调整列顺序',
  'table.dragRow': '拖拽调整行顺序',
  'table.rowCommit': '提交本行变更',
  'table.gutterCommit': '提交',
  'table.rowRollback': '撤销本行变更',
  'table.rowStatus': '行状态',
  'table.copyCell': '复制单元格',
  'table.copyRow': '复制行',
  'table.filterColumn': '筛选 {title}',
  'table.filterPlaceholder': '输入筛选…',
  'table.filterClear': '清除',
  'table.filterApply': '确定',
  'table.nullValue': '(NULL)',
  'table.setNull': '设为 NULL',
  'table.editHint': 'F2 编辑 · Tab 下一格 · Ctrl+0 NULL · Ctrl+Z 撤销',
  'table.editTextarea': '单元格多行文本',
  'upload.label': '点击或拖拽文件到此处上传',
  'upload.browse': '或点击浏览本地文件',
  'upload.download': '下载文件',
  'upload.dropActive': '松开以添加文件',
  'upload.button': '选择文件',
  'upload.add': '添加文件',
  'upload.preview': '预览文件',
  'upload.fileList': '已选文件',
  'calendarGrid.label': '月历',
  'calendarGrid.prevYear': '上一年',
  'calendarGrid.nextYear': '下一年',
  'calendarGrid.prevMonth': '上个月',
  'calendarGrid.nextMonth': '下个月',
  'calendarGrid.week': '周',
  'datePicker.placeholder': '选择日期',
  'datePicker.rangePlaceholder': '选择日期范围',
  'datePicker.clear': '清空日期',
  'datePicker.prevYear': '上一年',
  'datePicker.nextYear': '下一年',
  'datePicker.prevMonth': '上个月',
  'datePicker.nextMonth': '下个月',
  'datePicker.today': '今天',
  'datePicker.rangeToday': '今天',
  'datePicker.confirm': '确定',
  'datePicker.separator': ' ~ ',
  'datePicker.rangeStart': '开始日期',
  'datePicker.rangeEnd': '结束日期',
  'dateTimePicker.placeholder': '选择日期时间',
  'dateTimePicker.rangePlaceholder': '选择日期时间范围',
  'dateTimePicker.time': '时间',
  'dateTimePicker.now': '此刻',
  'dateTimePicker.clear': '清空',
  'dateTimePicker.rangeStart': '开始时间',
  'dateTimePicker.rangeEnd': '结束时间',
  'timePicker.placeholder': '选择时间',
  'timePicker.embeddedPlaceholder': '选择',
  'timePicker.rangePlaceholder': '选择时间范围',
  'timePicker.hour': '时',
  'timePicker.minute': '分',
  'timePicker.second': '秒',
  'timePicker.now': '此刻',
  'timePicker.rangePreset': '朝九晚六',
  'timePicker.clear': '清空',
  'timePicker.rangeClear': '清空',
  'timePicker.confirm': '确定',
  'timePicker.separator': ' ~ ',
  'timePicker.rangeStart': '开始时间',
  'timePicker.rangeEnd': '结束时间',
  'timePicker.am': '上午',
  'timePicker.pm': '下午',
  'timePicker.period': '上午/下午',
  'tree.label': '树形控件',
  'tree.expand': '展开节点',
  'tree.collapse': '收起节点',
  'tree.check': '勾选节点',
  'tree.empty': '暂无数据',
  'tree.drag': '拖拽排序',
  'split.label': '面板分割',
  'split.resize': '拖动调整面板尺寸',
  'split.value': '{value}%',
  'split.collapse': '收起面板',
  'split.expand': '展开面板',
  'drawer.label': '抽屉',
  'drawer.close': '关闭',
  'drawer.resize': '拖动调整抽屉尺寸',
  'terminal.label': '终端',
  'terminal.copy': '复制',
  'terminal.paste': '粘贴',
  'terminal.selectAll': '全选',
  'terminal.clear': '清空终端',
  'terminal.askAi': '询问 AI',
  'terminal.search': '搜索',
  'terminal.searchPlaceholder': '在终端中查找',
  'terminal.searchNext': '下一个',
  'terminal.searchPrev': '上一个',
  'terminal.searchClose': '关闭搜索',
  'terminal.searchEmpty': '没有匹配',
  'log.label': '日志',
  'log.empty': '暂无日志',
  'log.search': '搜索日志',
  'log.searchPlaceholder': '过滤正文…',
  'log.copy': '复制全文',
  'log.copyMatches': '复制匹配行',
  'log.copied': '已复制',
  'log.copyFailed': '复制失败，请用 HTTPS 或手动选择正文',
  'log.filter': '按级别过滤',
  'log.newLines': '{count, plural, other {新增 # 行}}',
  'log.lineNo': '第 {n} 行',
  'log.level.trace': '跟踪',
  'log.level.debug': '调试',
  'log.level.info': '信息',
  'log.level.notice': '通知',
  'log.level.success': '成功',
  'log.level.warn': '警告',
  'log.level.error': '错误',
  'log.level.fatal': '致命',
  'log.level.plain': '输出',
  'log.jump': '回到最新',
  'log.matchCount': '{count, plural, other {# 条匹配}}',
  'playground.brand': 'Niuma UI',
  'playground.subtitle': '内部测试，不是文档站',
  'playground.docs': '文档站',
  'playground.nav.label': '测试页导航',
  'playground.nav.start': '索引',
  'playground.nav.overview': '测试索引',
  'playground.nav.components': '组件',
  'playground.nav.basic': '基础',
  'playground.nav.form': '表单',
  'playground.nav.navigation': '导航与布局',
  'playground.nav.feedback': '浮层与反馈',
  'playground.nav.data': '数据展示',
  'playground.nav.editor': '编辑器',
  'playground.nav.lab': '像素回归',
  'playground.nav.search': '搜索组件…',
  'playground.nav.empty': '没有匹配的组件',
  'playground.nav.toggle': '打开导航',
  'playground.theme.label': '主题',
  'playground.theme.dark': 'Dark',
  'playground.theme.light': 'Light',
  'playground.locale.label': '语言',
  'playground.demo.testFile': '单元测试',
  'playground.demo.showCode': '查看代码',
  'playground.demo.hideCode': '隐藏代码',
  'playground.demo.api': 'API',
  'playground.demo.api.name': '属性',
  'playground.demo.api.type': '类型',
  'playground.demo.api.default': '默认值',
  'playground.demo.api.desc': '说明',
  'playground.demo.apiHint': '下表仅供回归对照。对外用法与 API 以文档站为准。',
  'playground.index.title': '内部组件测试台',
  'playground.index.intro': 'playground 只做冒烟与像素回归。安装、何时使用、API 只认 site/ 文档站。',
  'playground.index.cta.site': '打开文档站',
  'playground.index.cta.browse': '打开测试页',
  'playground.index.docs': '官方用法',
  'playground.index.docsHint': '组件文档：https://blair-shang.github.io/niuma-ui/ 。本页不维护对外 API。',
  'playground.index.featured': '常用测试页',
  'playground.index.featuredHint': '高频控件的交互冒烟入口。',
  'playground.index.dev': '测试对照表',
  'playground.index.devHint': '测试页与单元测试文件对照。',
  'playground.index.column.component': '组件',
  'playground.index.column.demo': '测试页',
  'playground.index.column.test': '单元测试',
  'playground.index.hint': '启动内部测试台',
  'codeBlock.copy': '复制',
  'codeBlock.copied': '已复制',
  'codeBlock.copyFailed': '复制失败',
  'codeBlock.download': '下载',
  'codeBlock.toolbar': '代码操作',
  'codeBlock.label': '代码',
  'markdown.edit': '编辑',
  'markdown.preview': '预览',
  'markdown.split': '分栏',
  'markdown.modes': 'Markdown 视图',
  'markdown.placeholder': '支持 Markdown（标题、列表、链接、代码块…）',
  'markdown.empty': '暂无内容',
  'markdown.label': 'Markdown',
  'markdown.taskDone': '已完成',
  'markdown.taskOpen': '未完成',
  'markdown.external': '在新标签页打开',
  'codeEditor.label': '代码编辑器',
  'codeEditor.diagnostics': '诊断',
  'codeEditor.inlineEdit': '行内编辑',
  'codeEditor.inlineEditPlaceholder': '描述要如何修改选区…',
  'codeEditor.inlineEditApply': '应用',
  'codeEditor.inlineEditApplying': '生成中…',
  'codeEditor.inlineEditFailed': '未能生成替换代码，请调整指令后重试',
}

export const enUS: RsLocaleMessages = {
  'select.placeholder': 'Select',
  'select.searchPlaceholder': 'Search…',
  'select.empty': 'No results',
  'select.loading': 'Loading…',
  'select.clear': 'Clear',
  'select.createOption': 'Use “{query}”',
  'select.maxTagPlaceholder': '+{count}',
  'select.listbox': 'Options',
  'mentions.placeholder': 'Type @ to mention',
  'mentions.suggestions': 'Mention suggestions',
  'autocomplete.placeholder': 'Type to search',
  'autocomplete.suggestions': 'Autocomplete suggestions',
  'dropdown.placeholder': 'Select',
  'dropdown.menu': 'Dropdown menu',
  'contextMenu.menu': 'Context menu',
  'contextMenu.submenu': 'Submenu',
  'breadcrumb.label': 'Breadcrumb',
  'breadcrumb.separator': 'Separator',
  'breadcrumb.more': 'Show more levels',
  'anchor.label': 'On this page',
  'steps.label': 'Steps',
  'steps.wait': 'Waiting',
  'steps.process': 'In progress',
  'steps.finish': 'Completed',
  'steps.error': 'Error',
  'menu.label': 'Navigation menu',
  'menu.expandSubmenu': 'Expand submenu',
  'menu.collapseSubmenu': 'Collapse submenu',
  'sidebar.label': 'Sidebar',
  'sidebar.nav': 'Sidebar navigation',
  'sidebar.expand': 'Expand sidebar',
  'sidebar.collapse': 'Collapse sidebar',
  'toolbar.label': 'Toolbar',
  'input.required': 'This field is required',
  'label.required': 'required',
  'label.optional': '(optional)',
  'label.colon': ':',
  'form.field': 'This field',
  'form.validate.required': 'Please enter {label}',
  'form.list.add': 'Add',
  'form.list.remove': 'Remove',
  'fieldset.help': 'Group help',
  'tooltip.help': 'Help',
  'popover.label': 'Popover',
  'popover.close': 'Close',
  'input.formatInvalid': 'Invalid format',
  'input.rule.email': 'Enter a valid email address',
  'input.rule.number': 'Enter a valid number',
  'input.rule.integer': 'Enter an integer',
  'input.rule.phone': 'Enter a valid 11-digit mobile number',
  'input.rule.url': 'Enter a valid URL starting with http:// or https://',
  'input.rule.minComplexity': 'Password must be at least 8 characters with letters and numbers',
  'input.clear': 'Clear',
  'input.showPassword': 'Show password',
  'input.hidePassword': 'Hide password',
  'inputNumber.increase': 'Increase',
  'inputNumber.decrease': 'Decrease',
  'common.confirm': 'Confirm',
  'common.cancel': 'Cancel',
  'common.remove': 'Remove',
  'dialog.close': 'Close',
  'dialog.fullscreen': 'Fullscreen',
  'dialog.restore': 'Restore',
  'dialog.label': 'Dialog',
  'tag.close': 'Close tag',
  'scrollbar.viewport': 'Scrollable area',
  'virtualList.empty': 'No items',
  'virtualList.label': 'Virtual list',
  'monaco.label': 'Code editor',
  'alert.close': 'Dismiss alert',
  'empty.description': 'No data',
  'descriptions.empty': '—',
  'empty.search': 'No matching results',
  'empty.label': 'Empty',
  'dynamicTags.label': 'Editable tags',
  'dynamicTags.placeholder': 'Type and press Enter',
  'dynamicTags.add': 'Add tag',
  'dynamicTags.clear': 'Clear tags',
  'dynamicTags.duplicate': 'Tag already exists',
  'dynamicTags.maxReached': 'Maximum number of tags reached',
  'dynamicTags.invalid': 'Invalid tag',
  'confirm.title': 'Confirm action?',
  'confirm.description': 'This action may not be reversible. Please confirm before continuing.',
  'loading.label': 'Loading',
  'statCard.loading': 'Loading',
  'statCard.trend.up': 'Increased',
  'statCard.trend.down': 'Decreased',
  'statCard.trend.flat': 'Unchanged',
  'loadingBar.label': 'Loading',
  'loadingBar.error': 'Loading failed',
  'loadingBar.indeterminate': 'Loading, progress unknown',
  'toaster.region': 'Notifications',
  'toaster.close': 'Dismiss notification',
  'toaster.loading': 'In progress',
  'toaster.done': 'Done',
  'toaster.failed': 'Something went wrong',
  'tabs.label': 'Tabs',
  'tabs.add': 'Add tab',
  'tabs.close': 'Close {label}',
  'tabs.closeTab': 'Close',
  'tabs.closeOthers': 'Close others',
  'tabs.closeLeft': 'Close to the left',
  'tabs.closeRight': 'Close to the right',
  'tabs.closeAll': 'Close all',
  'tabs.more': 'More',
  'tabs.moreCount': 'More ({count})',
  'tabs.rename': 'Rename tab',
  'tabs.renamePlaceholder': 'Enter tab name',
  'tabs.scrollPrev': 'Scroll tabs backward',
  'tabs.scrollNext': 'Scroll tabs forward',
  'tabs.dragHandle': 'Drag to reorder',
  'pagination.label': 'Pagination',
  'pagination.prev': 'Previous',
  'pagination.next': 'Next',
  'pagination.first': 'First',
  'pagination.last': 'Last',
  'pagination.page': 'page',
  'pagination.pageN': 'Page {page}',
  'pagination.current': 'Page {page}, current',
  'pagination.simple': '{page} / {pageCount}',
  'pagination.ellipsis': 'More pages',
  'pagination.summary': '{total, plural, one {# item} other {# items}}',
  'pagination.jumpTo': 'Go to',
  'pagination.jumpPageSuffix': 'page',
  'pagination.jumpConfirm': 'Go',
  'table.loading': 'Loading…',
  'table.empty': 'No data',
  'table.loadingMore': 'Loading more…',
  'table.a11yLabel': 'Data table',
  'table.selectAll': 'Select all',
  'table.selectRow': 'Select row',
  'table.index': '#',
  'table.expandRow': 'Expand row',
  'table.collapseRow': 'Collapse row',
  'table.dragColumn': 'Drag to reorder column',
  'table.dragRow': 'Drag to reorder row',
  'table.rowCommit': 'Commit row changes',
  'table.gutterCommit': 'Commit',
  'table.rowRollback': 'Revert row changes',
  'table.rowStatus': 'Row status',
  'table.copyCell': 'Copy cell',
  'table.copyRow': 'Copy row',
  'table.filterColumn': 'Filter {title}',
  'table.filterPlaceholder': 'Filter…',
  'table.filterClear': 'Clear',
  'table.filterApply': 'Apply',
  'table.nullValue': '(NULL)',
  'table.setNull': 'Set NULL',
  'table.editHint': 'F2 edit · Tab next · Ctrl+0 NULL · Ctrl+Z undo',
  'table.editTextarea': 'Cell multiline text',
  'upload.label': 'Click or drag files here to upload',
  'upload.browse': 'or click to browse files',
  'upload.download': 'Download file',
  'upload.dropActive': 'Drop files to upload',
  'upload.button': 'Choose files',
  'upload.add': 'Add files',
  'upload.preview': 'Preview file',
  'upload.fileList': 'Selected files',
  'calendarGrid.label': 'Calendar',
  'calendarGrid.prevYear': 'Previous year',
  'calendarGrid.nextYear': 'Next year',
  'calendarGrid.prevMonth': 'Previous month',
  'calendarGrid.nextMonth': 'Next month',
  'calendarGrid.week': 'Wk',
  'datePicker.placeholder': 'Select date',
  'datePicker.rangePlaceholder': 'Select date range',
  'datePicker.clear': 'Clear date',
  'datePicker.prevYear': 'Previous year',
  'datePicker.nextYear': 'Next year',
  'datePicker.prevMonth': 'Previous month',
  'datePicker.nextMonth': 'Next month',
  'datePicker.today': 'Today',
  'datePicker.rangeToday': 'Today',
  'datePicker.confirm': 'Confirm',
  'datePicker.separator': ' – ',
  'datePicker.rangeStart': 'Start date',
  'datePicker.rangeEnd': 'End date',
  'dateTimePicker.placeholder': 'Select date and time',
  'dateTimePicker.rangePlaceholder': 'Select date-time range',
  'dateTimePicker.time': 'Time',
  'dateTimePicker.now': 'Now',
  'dateTimePicker.clear': 'Clear',
  'dateTimePicker.rangeStart': 'Start time',
  'dateTimePicker.rangeEnd': 'End time',
  'timePicker.placeholder': 'Select time',
  'timePicker.embeddedPlaceholder': 'Select',
  'timePicker.rangePlaceholder': 'Select time range',
  'timePicker.hour': 'Hour',
  'timePicker.minute': 'Min',
  'timePicker.second': 'Sec',
  'timePicker.now': 'Now',
  'timePicker.rangePreset': '9:00 – 18:00',
  'timePicker.clear': 'Clear',
  'timePicker.rangeClear': 'Clear',
  'timePicker.confirm': 'Confirm',
  'timePicker.separator': ' – ',
  'timePicker.rangeStart': 'Start time',
  'timePicker.rangeEnd': 'End time',
  'timePicker.am': 'AM',
  'timePicker.pm': 'PM',
  'timePicker.period': 'AM/PM',
  'tree.label': 'Tree',
  'tree.expand': 'Expand node',
  'tree.collapse': 'Collapse node',
  'tree.check': 'Check node',
  'tree.empty': 'No data',
  'tree.drag': 'Drag to reorder',
  'split.label': 'Split panes',
  'split.resize': 'Drag to resize panes',
  'split.value': '{value}%',
  'split.collapse': 'Collapse pane',
  'split.expand': 'Expand pane',
  'drawer.label': 'Drawer',
  'drawer.close': 'Close',
  'drawer.resize': 'Drag to resize the drawer',
  'terminal.label': 'Terminal',
  'terminal.copy': 'Copy',
  'terminal.paste': 'Paste',
  'terminal.selectAll': 'Select All',
  'terminal.clear': 'Clear Terminal',
  'terminal.askAi': 'Ask AI',
  'terminal.search': 'Search',
  'terminal.searchPlaceholder': 'Find in terminal',
  'terminal.searchNext': 'Next',
  'terminal.searchPrev': 'Previous',
  'terminal.searchClose': 'Close search',
  'terminal.searchEmpty': 'No matches',
  'log.label': 'Log',
  'log.empty': 'No log output',
  'log.search': 'Search logs',
  'log.searchPlaceholder': 'Filter lines…',
  'log.copy': 'Copy all',
  'log.copyMatches': 'Copy matching lines',
  'log.copied': 'Copied',
  'log.copyFailed': 'Copy failed. Use HTTPS or select the log text.',
  'log.filter': 'Filter levels',
  'log.newLines': '{count, plural, one {# new line} other {# new lines}}',
  'log.lineNo': 'Line {n}',
  'log.level.trace': 'Trace',
  'log.level.debug': 'Debug',
  'log.level.info': 'Info',
  'log.level.notice': 'Notice',
  'log.level.success': 'Success',
  'log.level.warn': 'Warning',
  'log.level.error': 'Error',
  'log.level.fatal': 'Fatal',
  'log.level.plain': 'Output',
  'log.jump': 'Jump to latest',
  'log.matchCount': '{count, plural, one {# match} other {# matches}}',
  'playground.brand': 'Niuma UI',
  'playground.subtitle': 'Internal test — not the docs site',
  'playground.docs': 'Docs site',
  'playground.nav.label': 'Test page navigation',
  'playground.nav.start': 'Index',
  'playground.nav.overview': 'Test index',
  'playground.nav.components': 'Components',
  'playground.nav.basic': 'Basic',
  'playground.nav.form': 'Form',
  'playground.nav.navigation': 'Navigation & layout',
  'playground.nav.feedback': 'Overlay & feedback',
  'playground.nav.data': 'Data display',
  'playground.nav.editor': 'Editors',
  'playground.nav.lab': 'Visual regression',
  'playground.nav.search': 'Search components…',
  'playground.nav.empty': 'No matching components',
  'playground.nav.toggle': 'Open navigation',
  'playground.theme.label': 'Theme',
  'playground.theme.dark': 'Dark',
  'playground.theme.light': 'Light',
  'playground.locale.label': 'Language',
  'playground.demo.testFile': 'Unit test',
  'playground.demo.showCode': 'Show code',
  'playground.demo.hideCode': 'Hide code',
  'playground.demo.api': 'API',
  'playground.demo.api.name': 'Prop',
  'playground.demo.api.type': 'Type',
  'playground.demo.api.default': 'Default',
  'playground.demo.api.desc': 'Description',
  'playground.demo.apiHint': 'This table is for regression only. Public usage and API live on the docs site.',
  'playground.index.title': 'Internal component test bench',
  'playground.index.intro': 'playground is smoke and visual regression only. Install, when-to-use, and API live on the site/ docs.',
  'playground.index.cta.site': 'Open docs site',
  'playground.index.cta.browse': 'Open a test page',
  'playground.index.docs': 'Official usage',
  'playground.index.docsHint': 'Component docs: https://blair-shang.github.io/niuma-ui/ . This page does not maintain the public API.',
  'playground.index.featured': 'Frequent test pages',
  'playground.index.featuredHint': 'Smoke entries for high-traffic controls.',
  'playground.index.dev': 'Test map',
  'playground.index.devHint': 'Test pages mapped to unit-test files.',
  'playground.index.column.component': 'Component',
  'playground.index.column.demo': 'Test page',
  'playground.index.column.test': 'Unit test',
  'playground.index.hint': 'Start the internal test bench',
  'codeBlock.copy': 'Copy',
  'codeBlock.copied': 'Copied!',
  'codeBlock.copyFailed': 'Copy failed',
  'codeBlock.download': 'Download',
  'codeBlock.toolbar': 'Code actions',
  'codeBlock.label': 'Code',
  'markdown.edit': 'Edit',
  'markdown.preview': 'Preview',
  'markdown.split': 'Split',
  'markdown.modes': 'Markdown view',
  'markdown.placeholder': 'Markdown supported (headings, lists, links, code…)',
  'markdown.empty': 'Nothing to preview',
  'markdown.label': 'Markdown',
  'markdown.taskDone': 'Completed',
  'markdown.taskOpen': 'Not completed',
  'markdown.external': 'opens in a new tab',
  'codeEditor.label': 'Code editor',
  'codeEditor.diagnostics': 'Diagnostics',
  'codeEditor.inlineEdit': 'Inline edit',
  'codeEditor.inlineEditPlaceholder': 'Describe how to change the selection…',
  'codeEditor.inlineEditApply': 'Apply',
  'codeEditor.inlineEditApplying': 'Generating…',
  'codeEditor.inlineEditFailed': 'Could not generate a replacement. Adjust the instruction and try again.',
}

/** 社区语言包应对齐的 key 清单（与 en-US 表相同）。 */
export const rsLocaleMessageKeys = Object.keys(enUS)

