...
本文面向将 `@niuma/ui` 集成到宿主应用（桌面端、Web 后台、官网等）的工程团队。阅读前请先浏览仓库根目录 [README.md](../README.md)。
...
| CI / 可复现构建 | `git+https://github.com/Blair-Shang/niuma-ui.git#v0.1.0` | **必须**钉在 tag |
| 公开发布后（规划） | `"@niuma/ui": "^0.1.0"` | npm / GitHub Packages |
...
**私有仓库**：本机需已登录 GitHub（Git Credential Manager / PAT）；CI 需具备 `contents: read` 的 token，并配置给 `pnpm`/`git`。
...
1. 阅读目标版本的 CHANGELOG / Release 说明（开源后维护）。
2. 将依赖 tag 从 `#v0.1.0` 改为新 tag，执行 `pnpm install`。
...
**Q: CI 报无法克隆私有仓库？**  
A: 检查 `GITHUB_TOKEN` / PAT 权限与 `git` 凭据；确认 tag 存在（`v0.1.0`）。
...
**Q: 开源后许可证？**  
A: 尚未公开。接入方在 Private 阶段按内部授权使用；公开时以仓库 `LICENSE` 为准。
...
第三方产品接入时，建议固定 minor 版本区间，并在自有文档中记录所用 `@niuma/ui` tag，便于审计与回溯。
